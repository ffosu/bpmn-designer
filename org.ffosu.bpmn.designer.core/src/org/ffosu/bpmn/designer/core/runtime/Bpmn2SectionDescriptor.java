		...
		public Bpmn2SectionDescriptor(Bpmn2TabDescriptor td, IConfigurationElement e) {
			...
			try {
				...
				if ("default".equals(className)) { //$NON-NLS-1$
					sectionClass = new AbstractFixFlowPropertySection();
				...
				if (type!=null && !type.isEmpty()) {
					...
					for (String t : types) {
						...
						if (sectionClass instanceof AbstractFixFlowPropertySection) {
							((AbstractFixFlowPropertySection)sectionClass).addAppliesToClass(clazz);
						}
					}
				}
			} catch (Exception e1) {
